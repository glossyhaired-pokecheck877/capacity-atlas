...
export async function collectCodexAccount({ home, readJson = defaultReadJson, fetch = globalThis.fetch }) {
  let email = null;
  try {
...
export async function collectClaudeAccount({
  ...
  try {
    ...
    const requestUsage = () => fetch("https://api.anthropic.com/api/oauth/usage", {
      headers: {
...
export async function collectGrokAccount({ home, readJson = defaultReadJson, fetch = globalThis.fetch }) {
  let email = null;
  try {
    ...
    const parsed = parseGrokBillingResponse(Buffer.from(await response.arrayBuffer()));
    const window = {
      kind: "credits",
      title: "利用枠",
...
export async function collectDirectProviders({ homes, readJson = defaultReadJson, readKeychain, fetch = globalThis.fetch } = {}) {
  ...
  const collect = (provider, source) => {
    ...
    const options = { home: entry.home, readJson, fetch };
